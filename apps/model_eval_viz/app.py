...
from config import EX_STATES, entered_points, nwis, STREAMGAGE_SUBSET
...
def _get_data(_filepath: str) -> gpd.GeoDataFrame:
    """
    Read streamflow data from a .csv file and filter it based on the ‘gagesII_class==ref’.
...
dobs = pd.DataFrame({"datetime": [""], "0060_Mean": [""]})
...
def display_map(map: str) -> gv.WMTS:
    ...
    """
    basemap = base_map_options[map].opts(projection=ccrs.Mercator())
    return basemap
...
"""
not_available = pn.pane.Markdown(gray + '<div class ="gray"> Not available </div>', sizing_mode="stretch_width")
popup = pn.pane.Markdown(sizing_mode="stretch_width")


initial_load = pn.Tabs(('US Map', pn.pane.HoloViews(displayed_map * displayed_states * displayed_points)), ('Plots', not_available))
