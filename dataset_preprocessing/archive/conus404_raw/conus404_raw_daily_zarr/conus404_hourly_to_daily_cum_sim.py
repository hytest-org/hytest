...
def compute_daily(ds, var_list, st_idx, en_idx, chunks=None, var_type='instant'):
    ...
    if var_type == 'instant':
        ...
        ds_daily = ds_day_cnk[var_list].coarsen(time=24, boundary='trim').mean().chunk(chunks)
    elif var_type == 'cum60':
        ...
        ds_daily = ds_day_cnk[var_list].coarsen(time=24, boundary='trim').sum().chunk(chunks)
    elif var_type == 'cum_sim':
        ds_day_cnk = ds[var_list].isel(time=slice(st_idx+1, en_idx+1))
        print(f'    cum_sim: hourly range: {st_idx+1} ({ds_day_cnk.time.dt.strftime("%Y-%m-%d %H:%M:%S")[0].values}) to '
              f'{en_idx+1} ({ds_day_cnk.time.dt.strftime("%Y-%m-%d %H:%M:%S")[-1].values})')

        ds_max = ds_day_cnk[var_list].coarsen(time=24, boundary='trim').max()
        ds_min = ds_day_cnk[var_list].coarsen(time=24, boundary='trim').min()
        ds_daily = ds_max - ds_min
...
def main():
    ...
    parser.add_argument('-t', '--type', help='Integration type to compute',
                        choices=['cum60', 'cum_sim', 'cum_sim_bucket', 'instant'], default='instant')
    ...
    with Client(cluster) as client:
        ...
        for c_loop in range(args.loop):
            ...
            ds_day_cnk = ds[var_list].isel(time=slice(c_st+1, c_en+1))
            print(f'    cum_sim: hourly range: {c_st+1} ({ds_day_cnk.time.dt.strftime("%Y-%m-%d %H:%M:%S")[0].values}) to '
                  f'{c_en+1} ({ds_day_cnk.time.dt.strftime("%Y-%m-%d %H:%M:%S")[-1].values})')
            ...
            print(f'{ds_min.RECH.size=}')
            print(f'{ds_min.RECH.isel(time=slice(0, 72), x=500, y=500).values.max()=}')
            print(f'{ds_min.RECH.isel(time=slice(0, 72), x=500, y=500).values.min()=}')

            print(f'{ds_max.RECH.size=}')
            print(f'{ds_max.RECH.isel(time=slice(0, 72), x=500, y=500).values.max()=}')
            print(f'{ds_max.RECH.isel(time=slice(0, 72), x=500, y=500).values.min()=}')
